...
# User management
This section describes how to new users (intented for developers) can connect to the Kubernetes cluster.
In order connect to the cluster, a user must have installed the next dependencies:
 - awscli >= 1.16.154
 - kubectl >= 1.13 
 - maws

They also have to be part of the `aws_discourse_dev` LDAP group.


### Get EKS credentials and test access
First run $(maws), a new browser window will open, there click on `maws-discourse-developers`. Now you will have the right IAM role.
